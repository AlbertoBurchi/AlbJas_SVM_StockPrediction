...
#        numOfChunks = ((len(sequence) - winSize) / step) + 1
    ...
    def calculate_holding_logreturn(self, column_with_close, batch_size, look_ahead):
        """
        Uses the common formula of percent change
        """
        # need to shift the column in order to not count hte first few days bec there wasnt any data to make
        ...
        return np.log(float(array_of_closes[-1]) / float(array_of_closes[0]))
    ...
    def take_bid_stream_calculate_return(self, column_bid_stream, batch_size, look_ahead):
        ...
        array_of_closes = self.main_df[column_bid_stream.replace('bid_stream', "CLS")].tolist()
        ...
        for bid in array_of_bid_stream:
            ...
            if bid == -1 and holding == 1:
                ...
                try:
                    sell_price = array_of_closes[index + batch_size + look_ahead]
                except:
                    print ("failsed heres")
                    return self.array_of_returns
                # now its time to sell out and calc return
                self.array_of_returns.append(self.percent_change(sell_price, buy_price))
#                self.array_of_returns.append(self.log_return(sell_price, buy_price))
...
#    def take_bid_stream_calculate_log_return(self, column_bid_stream, batch_size, look_ahead):
#        """
#        This feature was created by Alberto ... what a thrill!
#        """
##        array_of_bid_stream = self.main_df[column_bid_stream].tolist()
#        array_of_bid_stream = self.main_df[column_bid_stream].tolist()[batch_size + look_ahead:]
...
#
##        array_of_closes = self.main_df[column_bid_stream.replace('bid_stream', "CLS")].tolist()
#        array_of_closes = self.main_df[column_bid_stream.replace('bid_stream', "CLS")].tolist()[batch_size + look_ahead:]
#        
...
#
#            if bid == -1 and holding == 1:
#                # cannot happen the first time because it starts at a 0
#                # this is when we should sell the stock after a bunch of 1's
#
#                try:
##                    sell_price = array_of_closes[index + batch_size + look_ahead]
#                    sell_price = array_of_closes[index]
#                except:
#                    print ("failsed heres")
#                    return self.array_of_returns
#                # now its time to sell out and calc return
##                self.array_of_returns.append(self.log_return(sell_price, buy_price))
#                self.array_of_returns.append(self.log_return(array_of_closes[index], array_of_closes[index-1]))
#                
#                holding = 0
#
#            if bid == 1 and holding == 1:
...
#        print (array_of_bid_stream, ' here is bid from fucntion')
        ...
        #######
        """
        HORRIBLE SOLUTION BUT AT THE MOMENT I DON'T KNOW HOW TO DO ...
        """
        ...
        while index < len(array_of_bid_stream):
            ...
            try:
                ...
                if holding == 1:
                    self.array_of_returns.append(self.log_return(array_of_closes[index], buy_price))
                return self.array_of_returns
...
#                        self.array_of_profits.append(array_of_closes[index])
                ...
                is_there_a_one, close_value = self.check_if_prior_days_have_a_buy_signal(array_of_bid_stream, array_of_closes, index - 1)

                while not is_there_a_one:
                    index = index - 1
                    is_there_a_one, close_value = self.check_if_prior_days_have_a_buy_signal(array_of_bid_stream, array_of_closes, index)
                ...
                for _ in range(look_ahead - 1):
                    ...
                    self.array_of_returns.append(0.000000001)
...
#        print (array_of_bid_stream, ' here is bid from fucntion')
        ...
        while index < len(array_of_bid_stream):
            ...
            try:
                bid = int(array_of_bid_stream[index])
                print ("Start - index {} | bid {} | holding {} | close {} | buy_price {} | sell_price {} | arr-prof{}".format(str(index), str(bid), str(holding), str(array_of_closes[index]), str(buy_price), str(sell_price), str(self.array_of_profits) ))

            except:
                print ("it failed on NONE Value")
                if holding == 1:
                    self.array_of_profits.append(array_of_closes[index] - buy_price)
                return self.array_of_profits
            ...
            if bid == 1 and holding == 0:
                ...
                # we dont want it to check the stock inbween the look ahead day and the day we got the sell signal
                # were just going to hold out and append 0 until we sell
                print ('jason here is the sell price', sell_price)
            ...
            # print ("bid: {} - buy_price: {} - sell_price: {} - holding: {}".format(str(bid), str(buy_price), str(sell_price), str(holding)))
            # print ("End - index {} | bid {} | holding {} | close {} | buy_price {} | sell_price {} | arr-prof{}".format(str(index), str(bid), str(holding), str(array_of_closes[index]), str(buy_price), str(sell_price),str(self.array_of_profits)))
    ...
    def take_bid_stream_calculate_profit_jason2020(self, column_bid_stream, batch_size, look_ahead, for_graph=False):
        """
        This function take a column with a bid stream then reads the bid stream
        then looks at the corospoiding close prices and caculates our return
        """
...
#        print (array_of_bid_stream, ' here is bid from fucntion')
        ...
        while index < len(array_of_bid_stream):
            ...
            if bid == 1 and holding == 0:
                ...
                # TODO dont think the next line is right
                self.array_of_profits.append(array_of_closes[index] - array_of_closes[index - 1])
