# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 11:53:36 2019
...
class Support_Vector():
    ...
    def train(self, keep=False):
        ...
        #svclassifier = SVC(C=4.426693, kernel='rbf', gamma=10**(-2.831807))
#        svclassifier = SVC(kernel='rbf', class_weight='balanced')
        ...
        confustion_matrix = confusion_matrix(self.y_test, self.y_pred) #Compute confusion matrix to evaluate the accuracy of a classification.

        print('confustion_matrix')
        print(confustion_matrix)

        print('classification_report')
        classification_reports = classification_report(self.y_test, self.y_pred,output_dict=True)
        print(classification_reports) #Build a text report showing the main classification metrics
        print (classification_reports['weighted avg'])

        precision = classification_reports['weighted avg']['precision']
        recall = classification_reports['weighted avg']['recall']
        f1_score = classification_reports['weighted avg']['f1-score']
        TruePositive = confustion_matrix[0,0]
        FalsePositive = confustion_matrix[0,1]
        FalseNegative = confustion_matrix[1,0]
        TrueNegative = confustion_matrix[1,1]
        ...
        if keep:
            joblib.dump(svclassifier, 'models/model' + str(datetime.now()).replace(':', '.') + '.pkl')
            

        return precision, recall, f1_score, classification_reports, confustion_matrix, TruePositive, FalsePositive, FalseNegative, TrueNegative  # , confustion_matrix[1]
    ...
    def train_model(self, x_train, y_train, kernel, C, logGamma, degree, coef0):
        ...
        if kernel == 'linear':
            ...
            raise "Unknown kernel function: %s"
    ...
    def run_optunity(self):
        ...
        print('calculating optimal hyper Parameters')
        optimal_svm_pars, info, _ = optunity.maximize_structured(svm_tuned_auroc, self.space, num_evals=5, pmap=optunity.pmap)
        print(("Optimal parameters" + str(optimal_svm_pars)))
        print(("AUROC of tuned SVM: %1.3f" % info.optimum))

        df = optunity.call_log2dataframe(info.call_log)
        print(df.sort_values('value', ascending=False))
        sorted_optunity =  df.sort_values('value', ascending=False)
        print (sorted_optunity)
        df.to_csv('sorted_optunity.csv')
