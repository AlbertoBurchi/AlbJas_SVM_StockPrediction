...
def main(batch_size, look_ahead):
    ...
    ptf_holding_final_capital, ptf_algo_final_capital = return_calculator.ptfvalue(batch_size, look_ahead)

    return ticker_data, columns,columns_with_sample_slopes,tdmdf,y_values,x_values, precision, recall, f1_score, classification_reports, confustion_matrix, TruePositive, FalsePositive, FalseNegative, TrueNegative,ptf_holding_final_capital, ptf_algo_final_capital
...
def iterate_over_all_batch_and_look_ahead():
    ...
    for j in range(1, 5):      
        for i in range(1, 21):
           ticker_data, columns,columns_with_sample_slopes,tdmdf,y_values,x_values, precision, recall, f1_score, classification_reports, confustion_matrix, TruePositive, FalsePositive, FalseNegative, TrueNegative,ptf_holding_final_capital, ptf_algo_final_capital=main(i, j)
           with open('results/iteration_performance.csv', mode='a') as target_file:
                   target_files = csv.writer(target_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

                   target_files.writerow([precision, recall, f1_score,TruePositive, FalsePositive, FalseNegative, TrueNegative,ptf_holding_final_capital, ptf_algo_final_capital,i,j])
